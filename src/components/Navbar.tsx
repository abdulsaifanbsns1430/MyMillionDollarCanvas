...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const percentageClaimed = ((totalClaimedPixels / 4000000) * 100).toFixed(2);
  const totalValuation = (totalClaimedPixels * 0.25).toLocaleString('en-US', {
    style: 'currency',
    currency: 'USD',
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <div className="bg-[#FFE169] border-[2.5px] border-black shadow-[3px_3px_0px_#000] px-3 py-1 rounded-xl flex items-center gap-2">
          <div className="w-3 h-3 bg-[#FF6B6B] border border-black rounded-sm animate-pulse" />
        ...
        {/* Live Metrics Pill */}
        <div className="hidden lg:flex items-center gap-2 bg-white border-[2px] border-black shadow-[2px_2px_0px_#000] px-3 py-1 rounded-xl text-xs font-semibold">
          <Layers className="w-3.5 h-3.5 text-[#5F27CD]" />
          <span className="text-gray-600">Claimed:</span>
          <span className="font-mono font-bold text-black">
            {totalClaimedPixels.toLocaleString()} / 4,000,000 px ({percentageClaimed}%)
          </span>
          <span className="text-gray-300">|</span>
          <span className="bg-[#FFE169] text-black font-bold px-1.5 py-0.2 rounded border border-black text-[11px]">
            $0.25 / px (4 px = $1)
          </span>
          <span className="text-gray-300">|</span>
